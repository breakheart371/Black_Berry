--Begin Tools.lua :)
local SUDO = 229445008 -- put Your ID here! <===
...
if is_sudo(msg) then
   		if matches[1]:lower() == 'add' or matches[1] == 'نصب' and not redis:get('ExpireDate:'..msg.to.id) then
			redis:set('ExpireDate:'..msg.to.id,true)
...
return chat_list(msg)
   ...
   if ((matches[1]:lower() == 'join') or (matches[1] == "افزودن")) and is_admin(msg) and matches[2] then
    ...
    tdcli.addChatMember(matches[2], msg.from.id, 0, dl_cb, nil)
		...
		if matches[1] == 'rem' or matches[1] == "لغونصب" and matches[2] and is_admin(msg) then
    local data = load_data(_config.moderation.data)
...
if matches[1] == 'blackTG' or matches[1] == "بلک تی جی" then
...
_Sudoer And Admins BlackTG Bot Help :_
...
text = [[
_راهنمای ادمین و سودو های ربات بِلَک:_
...
if matches[1] == "راهنمای ابزار" and is_mod(msg) then
if not lang then
text = [[
