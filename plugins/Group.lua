--Begin GroupManager.lua By @sudo_hacker
...
local lang = redis:get(hash)
   ...
   if not lang then
     ...
     return '》 _شما_ #ادمین ربات نیستید_ 🚷\n*〰〰〰〰〰〰〰〰*\n🗯اجرای این دستور فقط برای مالکان و مدیران است.'
    end
end
...
local lang = redis:get(hash)
 ...
 if not lang then
  ...
  return '》 _گروه از_ #لیست _گروه های ربات حذف شد_ 💠\n*〰〰〰〰〰〰〰〰*\n🗯گروه هم اکنون از لیست گروه ربات حذف شد.'
end
end
...
local lang = redis:get(hash)
  ...
  if not lang then
   ...
   return "》 _در گروه_ #مدیری _وجود نداره_ ‼️"
  end
end
if not lang then
   message = '⚜*lłšт øf мøđeяαтøя 👥 :*\n'
else
   message = '⚜*لیست مدیران های گروه👥 :*\n'
end
...
return "》 _در گروه_ #مالکی _وجود نداره_ ‼️\n*〰〰〰〰〰〰〰〰*\n🗯در این‌ گروه در حال حاظر هیچ مالکی انتخاب نشده است.\n نام :"..msg.from.first_name.." \n یوزنیم : [@"..(check_markdown(msg.from.username) or 'یوز نیم موجود نیست').."]"
  end
end
  -- determine if table is empty
 ...
 if not lang then
    ...
    return "#》هیچ مالکی برای ربات در این گروه انتخاب نشده است❕👤"
  end
end
if not lang then
   message = '⚜*lłšт øf мøđeяαтøя 👥 :*\n'
...
if administration[tostring(arg.chat_id)]['owners'][tostring(data.id_)] then
    ...
    return tdcli.sendMessage(arg.chat_id, "", 0, "》👤کاربر"..user_name.." *"..data.id_.."* *از قبل مالک بود*⚠️", 0, "md")
      end
   end
administration[tostring(arg.chat_id)]['owners'][tostring(data.id_)] = user_name
    save_data(_config.moderation.data, administration)
  ...
  return tdcli.sendMessage(arg.chat_id, "", 0, "》👤کاربر "..user_name.." *"..data.id_.."* *مالک گروه شد*✔️", 0, "md")
   end
end
tdcli_function ({
    ID = "GetUser",
...
return tdcli.sendMessage(arg.chat_id, "", 0, "》👤کاربر "..user_name.." *"..data.id_.."* *از قبل مالک نبود*❌", 0, "md")
      end
   end
...
return tdcli.sendMessage(arg.chat_id, "", 0, "》👤کاربر "..user_name.." *"..data.id_.."* *از مقام مالک برکنار شد*✔️", 0, "md")
   end
end
tdcli_function ({
    ID = "GetUser",
...
if not administration[tostring(arg.chat_id)]['mods'][tostring(data.id_)] then
    ...
    return tdcli.sendMessage(arg.chat_id, "", 0, "》👤کاربر "..user_name.." *"..data.id_.."* *از قبل مدیر نبود*❗️", 0, "md")
   end
  end
administration[tostring(arg.chat_id)]['mods'][tostring(data.id_)] = nil
   ...
   if not lang then
    ...
    return tdcli.sendMessage(arg.chat_id, "", 0, "》👤کاربر "..user_name.." *"..data.id_.."* *از مقام مدیر گروه برکنار شد*✔️", 0, "md")
   end
end
tdcli_function ({
    ID = "GetUser",
...
administration[tostring(arg.chat_id)]['owners'][tostring(data.id_)] = user_name
  ...
  return tdcli.sendMessage(arg.chat_id, "", 0, "》👤کاربر "..user_name.." *"..data.id_.."* *مالک گروه شد*✔️", 0, "md")   end
end
  if cmd == "promote" then
if administration[tostring(arg.chat_id)]['mods'][tostring(data.id_)] then
   if not lang then
...
elseif lang then
 return "بیصدا کردن #سرویس تلگرام از قبل فعال است🔇\n*〰〰〰〰〰〰〰〰*\nسفارش توسط : [@"..(check_markdown(msg.from.username or NoUseRNaMe)).."]"
end
else
...
return "بیصدا کردن #سرویس تلگرام  فعال شد🔇\n*〰〰〰〰〰〰〰〰*\nسفارش توسط : [@"..(check_markdown(msg.from.username or NoUseRNaMe)).."]"
end
end
end
...
elseif lang then
return "بیصدا کردن #سرویس تلگرام فعال نیست🔊\n*〰〰〰〰〰〰〰〰*\nسفارش توسط : [@"..(check_markdown(msg.from.username or NoUseRNaMe)).."]"
end 
else 
...
return "بیصدا کردن #سرویس تلگرام  غیرفعال شد🔊\n*〰〰〰〰〰〰〰〰*\nسفارش توسط : [@"..(check_markdown(msg.from.username or NoUseRNaMe)).."]"
end 
end
end
...
if matches[1] == "setowner" and is_admin(msg) or  matches[1] =="مالک" and is_admin(msg) or matches[1] == "Setowner" and is_admin(msg) then
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
...
if matches[1] == "promote" and is_owner(msg) or matches[1] =="کمک مدیر" and is_owner(msg) or matches[1] == "Promote" and is_owner(msg) then
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.to.id,
...
if matches[1]:lower() == 'setchar' and is_mod(msg) or matches[1]:lower() == 'Setchar'  and is_mod(msg) or matches[1]:lower() == 'تعدادحروف' and is_mod(msg) then
			if not is_mod(msg) then
				return
			end
  ...
  if matches[1] == 'setflood' and is_mod(msg) or matches[1] == 'تعدادفلود' and is_mod(msg) or matches[1] == 'Setflood' and is_mod(msg) then
  ...
  if matches[1]:lower() == 'setfloodtime' and is_mod(msg) or matches[1]:lower() == 'Setfloodtime' and is_mod(msg) or matches[1] == 'زمان فلود' and is_mod(msg) then
			if tonumber(matches[2]) < 1 or tonumber(matches[2]) > 10 then
				return "_Wrong number, range is_ *[1-10]*"
      end
...
if matches[1] == "modlist" or matches[1] == "لیست مدیران" or matches[1] == "Modlist" then
return modlist(msg)
end
...
`{لینک ، یوزرنیم ، هشتگ ، فراخوانی ، اسپم ، حساسیت ، ویرایش ، ربات ، فونت ، وب ، سنجاق ، انگلیسی ، عربی ، ویو ، شکلک ، تبلیغات ، فحش ، دستورات ، ورود}`
...
`{همه ، گیف ، چت ، عکس ، فیلم ، اهنگ ، ویس ، استیکر ، مخاطب ، فوروارد ، کیبورد ، فایل ، مکان ، سرویس تلگرام ، بازی ، 
اینلاین}`

🔊*باصدا *

`{همه ، گیف ، چت ، عکس ، فیلم ، اهنگ ، ویس ، استیکر ، مخاطب ، فوروارد ، کیبورد ، فایل ، مکان ، سرویس تلگرام ، بازی ، اینلاین}`
...
💥تنظیم فرد به عنوان مدیر ربات در گروه

*Demote* `[username , id , reply]`
💥تنزل مقام فرد از مدیر ربات در گروه
...
⚡تنزل مقام فرد از مدیر ربات برای گروه

*تنظیمات*
⚡️دریافت لیست تنظیمات

*لیست مدیران*
💥دریافت لیست کمک مدیران گروه
...
✳️_برای گروه های 0 تا 5000 نفر ‌_
`➰1 ماهه  15 هزار تومان`
`➰3 ماهه  35 هزار تومان`
`➰دائمی    50 هزار تومان`
*〰〰〰〰〰〰〰*
1⃣._توجه داشته باشید به هیچ وجه نباید ربات رو از گروه حذف کنید و در صورت داشتن هرگونه مشکل به بنده یاگروه پشتیبانی مراجعه کنید._

2⃣._لازم به ذکره پرداخت به صورت کارت به کارت میباشد._

3⃣._درصورت پاک کردن ربات به هردلیلی ازگروه خود جهت ادد زدن مجدد باید نصف هزینه خرید رو پرداخت کنید._
*________________*
...
*> زبان ربات فارسی !*
_برای تغییر زبان_ `[تنظیم زبان]` `[فارسی , انگلسیی]`
_داخل پرانتز ها به جای ساعت،دقیقه،ثانیه عدد موردنظربزارید_
➖〰➖〰➖〰➖〰➖
💻 *ρσωєяɗ Ɓу :* @sudo\_hacker
🎭 *ƇнαηηєƖ :* @blackandwhiteTM
]]
